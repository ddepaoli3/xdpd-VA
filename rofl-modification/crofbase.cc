		...
		crofctl* ctl_passed)
{
	...
	if (ctl_passed == NULL)// Case with ctl null
	{
	...
	{
		if (not ctl_passed->is_established()) {
			throw eRofBaseNotConnected();
		}

		switch (ctl_passed->get_version()) {
		case rofl::openflow::OFP_VERSION_UNKNOWN: {
			throw eRofBaseNotConnected();
		} break;
		case rofl::openflow12::OFP_VERSION: {

			switch (ctl_passed->get_role().get_role()) {
			case rofl::openflow13::OFPCR_ROLE_SLAVE: {
				throw eRofBaseNotConnected();
			} break;
			default: {
		...
		case rofl::openflow13::OFP_VERSION: {
			...
			case rofl::openflow13::OFPCR_ROLE_EQUAL:
			case rofl::openflow13::OFPCR_ROLE_MASTER: {
				if (not (ctl_passed->get_async_config().get_packet_in_mask_master() & (1 << reason))) {
					throw eRofBaseNotConnected();
				}
			} break;
			case rofl::openflow13::OFPCR_ROLE_SLAVE: {
				if (not (ctl_passed->get_async_config().get_packet_in_mask_slave() & (1 << reason))) {
					throw eRofBaseNotConnected();
				}
			} break;
			default: {
		...
		default: {
			...
			ctl_passed->send_packet_in_message(
					buffer_id,
					total_len,
					reason,
					table_id,
